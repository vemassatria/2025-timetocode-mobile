...
class MatrixGameController extends StateNotifier<MatrixGameState> {
  ...
  void nextLevel() {
    ...
    final currentIndex = allLevels.indexWhere((l) => l.levelId == state.level.levelId);
    final nextIndex = currentIndex + 1;

    if (nextIndex < allLevels.length) {
      final newLevel = allLevels[nextIndex];
      _ref.read(routerProvider).pushReplacement('/matriks/level', extra: newLevel);
    } else {
      exitGame();
    }
  }
  ...
  void exitGame() {
    _ref.read(routerProvider).go('/matriks');
  }
