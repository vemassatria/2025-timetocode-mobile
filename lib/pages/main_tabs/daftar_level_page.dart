...
class _DaftarLevelPageState extends State<DaftarLevelPage> {
  ...
  void _checkVisibility() {
    ...
    for (int i = 0; i < _cardKeys.length; i++) {
      ...
      final cardPosition = cardBox.localToGlobal(Offset.zero, ancestor: renderBox);
                    ...
                    child: LevelCard(
                      ...
                      onStartPressed: () {
                        if (index == 0) {
                          ...
                          (widget.game as GameEngine).loadCharacter('kakak', 'kating');
                          widget.game.overlays.remove('GameUI');
                          widget.game.overlays.add('Story');
                        } else {
                          debugPrint("Start button pressed for level ${index + 1}");
                        }
                      },
                      onInfoPressed: () {
